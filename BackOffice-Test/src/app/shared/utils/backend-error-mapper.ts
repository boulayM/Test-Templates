...
export function mapBackendError(err: unknown, fallbackMessage: string): MappedBackendError {
  ...
  const backendMessage = typeof e.error?.message === 'string' ? e.error.message : '';
  const lowerBackendMessage = backendMessage.toLowerCase();
  const isDuplicateEmail =
    lowerBackendMessage.includes('email') &&
    (lowerBackendMessage.includes('deja') ||
      lowerBackendMessage.includes('already') ||
      lowerBackendMessage.includes('exist') ||
      lowerBackendMessage.includes('used') ||
      lowerBackendMessage.includes('utilise'));

  let message = fallbackMessage;
  if (e.status === 409 || e.error?.code === 'CONFLICT' || isDuplicateEmail) {
